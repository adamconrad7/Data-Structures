...
int stack_isempty(struct stack* stack) {
  assert(stack);
  if(get_val(stack->list) == 1){
    return 1;
  }else{
    return 0;
  }
  // assert(stack->list);
  // assert(get_head(stack->list));
//  printf("\n\n%ul\n\n",stack->list);
//   assert(stack->list->head);
// //  printf("\n\nhere\n\n");
//   if(stack->list->head->val == NULL){
//     return 1;
//   }else{
//     return 0;
//   }
...
void stack_push(struct stack* stack, void* val) {
  //stack->list->head->next = stack->list->head;
...
void* stack_pop(struct stack* stack) {
  //must first use stack_top to identify which value to be removed and then pass that as arg to
  //list_remove
  void* val = stack_top(stack);
  list_remove(stack->list, val, &compare);
  return val;
}
